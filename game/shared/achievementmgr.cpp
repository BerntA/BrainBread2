...

{
    ...
    if ( bPersistToSteamCloud )
    {
		...
		ISteamRemoteStorage *pRemoteStorage = (steamapicontext && steamapicontext->SteamClient()) ?
			(ISteamRemoteStorage *)steamapicontext->SteamClient()->GetISteamGenericInterface(SteamAPI_GetHSteamUser(), SteamAPI_GetHSteamPipe(), STEAMREMOTESTORAGE_INTERFACE_VERSION) : NULL;
        if (pRemoteStorage)
        {
...
#ifdef CLIENT_DLL
	else if ( 0 == Q_strcmp( name, "death_notice" ) )
	{
		CBaseEntity *pVictim = ClientEntityList().GetEnt(event->GetInt("victimID"));
		CBaseEntity *pAttacker = ClientEntityList().GetEnt(event->GetInt("killerID"));
