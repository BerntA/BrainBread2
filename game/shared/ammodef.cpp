...
void CAmmoDef::AddAmmoType(char const* name, int damageType, int tracerType, int plr_dmg, char const* npc_var, int carry, float physicsForceImpulse, int nFlags, int minSplashSize, int maxSplashSize)
{
	if (AddAmmoType(name, damageType, tracerType, nFlags, minSplashSize, maxSplashSize) == false)
		return;
	...
	if (npc_var)
	{
		m_AmmoType[m_nAmmoIndex].pNPCDmgCVar = cvar->FindVar(npc_var);
		if (!m_AmmoType[m_nAmmoIndex].pNPCDmgCVar)
		{
			Msg("ERROR: Ammo (%s) found no CVar named (%s)\n", name, npc_var);
		}
