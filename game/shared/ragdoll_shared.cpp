...
void C_BaseAnimating::IgniteRagdoll( C_BaseAnimating *pSource )
{
	...
	if ( pChild )
	{
		...
		C_ClientRagdollGib *pClientRagdoll = dynamic_cast< C_ClientRagdollGib * > (this);
		if ( pFireChild )
		{
			if (pRagdoll)
			...
			if (pClientRagdoll)
				pClientRagdoll->SetEffectEntity(FireEffect(pClientRagdoll, pFireChild, NULL, NULL, NULL));
		}
	}
}
