...
public:
	...
	virtual bool		SpawnRunSchedule(CBaseEntity *pTarget, Activity act, bool pathcorner, int interruptType);
...
public:
	...
	COutputEvent        m_OnDeathByHeadshot;
...
public:
	...
	void				SetFrozen(bool state = false);
...
private:
	...
	float m_flSoundPenalty;

	// Freezing Logic:
	MoveType_t m_LastMoveType;
	float m_flLastGravity;

public:

	const char *GetFriendlyName(void) const { return m_szNPCName; }
	void SetNPCName(const char *name)  { Q_strncpy(m_szNPCName.GetForModify(), name, MAX_MAP_NAME_SAVE); }
	void SetBoss(bool bBoss) { m_bIsBoss = bBoss; }
	...
	void SetSoundPenaltyTime(float delay) { m_flSoundPenalty = gpGlobals->curtime + delay; }
