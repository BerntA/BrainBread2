...
{
	if (!GetOuter()->OverrideShouldAddToLookList(pSightEnt))
		return false;

	if (GetOuter() && GetOuter()->CanAlwaysSeePlayers() && pSightEnt->IsPlayer())
	...
	CNPC_BaseZombie *pBaseZombie = dynamic_cast<CNPC_BaseZombie*> (GetOuter());
	if (pBaseZombie && !pBaseZombie->IsBoss())
		bIsZombie = true;

	if ((ShouldSeeEntity(pSightEnt) && CanSeeEntity(pSightEnt)) || (bIsZombie && pSightEnt->IsPlayer() && GetOuter()->FVisible(pSightEnt, MASK_VISIBLE)))
		return SeeEntity(pSightEnt);
