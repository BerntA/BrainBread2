	...
	if ( !m_bDidDeathCleanup )
	{
		...
		if( bFireDeathOutput )
		{
			...
			if (LastHitGroup() == HITGROUP_HEAD)
				m_OnDeathByHeadshot.FireOutput(pCulprit, this);
		}
...
float CAI_BaseNPC::GetHitgroupDamageMultiplier( int iHitGroup, const CTakeDamageInfo &info )
{
	...
	case HITGROUP_HEAD:
		return GameBaseShared()->GetNPCData()->GetLimbData(GetFriendlyName(), "head");

	case HITGROUP_CHEST:
		return GameBaseShared()->GetNPCData()->GetLimbData(GetFriendlyName(), "chest");

	case HITGROUP_STOMACH:
		return GameBaseShared()->GetNPCData()->GetLimbData(GetFriendlyName(), "stomach");

	case HITGROUP_LEFTARM:
	case HITGROUP_RIGHTARM:
		return GameBaseShared()->GetNPCData()->GetLimbData(GetFriendlyName(), "arm");

	case HITGROUP_LEFTLEG:
	case HITGROUP_RIGHTLEG:
		return GameBaseShared()->GetNPCData()->GetLimbData(GetFriendlyName(), "leg");
...
CBaseEntity *CAI_BaseNPC::EyeLookTarget( void )
{
	if (m_flNextEyeLookTime < gpGlobals->curtime)
	{
		...
		for ( CEntitySphereQuery sphere( GetAbsOrigin(), 1024, 0 ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
		{
			...
			{
				// Zombie NPCs should not look towards player zombies.
				if (pNPC && pEntity->IsPlayer())
				{
					if (pNPC->IsZombie(true) && pEntity->IsZombie())
						continue;
				}
...
Activity CAI_BaseNPC::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	{
		if ((eNewActivity == ACT_COVER) || ((eNewActivity == ACT_IDLE) && HasMemory(bits_MEMORY_INCOVER)))
		{
			if (SelectWeightedSequence(ACT_COVER) == ACTIVITY_NOT_AVAILABLE) // Not all npcs have cover act.
				return ACT_IDLE;
			return ACT_COVER;
		}
	}
...
BEGIN_DATADESC( CAI_BaseNPC )
	...
	DEFINE_OUTPUT(m_OnDeathByHeadshot, "OnDeathByHeadshot"),
...
void CAI_BaseNPC::SetFrozen(bool state)
{
	// Freeze
	if (state)
	{
		m_LastMoveType = GetMoveType();
		m_flLastGravity = GetGravity();
	...
	SetMoveType(m_LastMoveType);
	SetGravity(m_flLastGravity);
}
...
void CAI_BaseNPC::TestPlayerPushing( CBaseEntity *pEntity )
{
	...
	if (pPlayer && !(pPlayer->GetFlags() & FL_NOTARGET) && (this->IRelationType(pPlayer) == D_LI))
	{
...
void CAI_BaseNPC::OnSkillFlagState(int flag, bool state)
{
	if (state)
	{
		if (flag == SKILL_FLAG_CRIPPLING_BLOW)
		{
			if (IsMoving())
				GetMotor()->MoveStop();
	...
	if ((flag == SKILL_FLAG_COLDSNAP) && IsMoving())
	{
		GetMotor()->MoveStop();
		GetMotor()->MoveStart();
	}
