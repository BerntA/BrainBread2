...
void CAI_BlendedMotor::BuildVelocityScript( const AILocalMoveGoal_t &move )
{
	...
	while (pCurWaypoint && (pCurWaypoint->NavType() == NAV_GROUND || pCurWaypoint->NavType() == NAV_FLY || pCurWaypoint->NavType() == NAV_CRAWL) /*&& flTotalDist / idealVelocity < 3.0*/) // limit lookahead to 3 seconds
	{
		script.Init();
