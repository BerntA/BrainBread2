...
CBasePlayer *BotPutInServer( bool bFrozen, int iTeam )
{
	...
	char botname[ 64 ];
	Q_strncpy(botname, pszBotNames[random->RandomInt(0, (_ARRAYSIZE(pszBotNames) - 1))], 64);
	...
	pPlayer->PickDefaultSpawnTeam(iTeam);
	
	CBaseCombatWeapon *pWeapon = (CBaseCombatWeapon*)pPlayer->GiveNamedItem("weapon_ak47");
	if (pWeapon)
	{
