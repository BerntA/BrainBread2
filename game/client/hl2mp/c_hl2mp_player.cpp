...
{
	/*
	const char* y = "yes sir I do obey the law.";
	unsigned int compressedSize;

	unsigned char* pCompressedBuffer = LZMA_Compress((unsigned char*)y, Q_strlen(y), &compressedSize);


	unsigned char out[32];
	unsigned char* pOut = out;
	unsigned char** ppOut = &(pOut);
	unsigned int size;


	bool worked = LZMA_Uncompress(pCompressedBuffer, &(pOut), &size);
	const char* str = reinterpret_cast<const char*>(pOut);
