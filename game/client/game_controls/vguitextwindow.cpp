...
void CTextWindow::Update( void )
{
	...
	m_pHTMLMessage->GetSize(wz, hz);
	m_pHTMLMessage->SetPos((w / 2) - (wz / 2), (h / 2) - (hz / 2));
	m_pHTMLMessage->GetPos(x, y);

	m_pTextMessage->GetSize(wz, hz);
	m_pTextMessage->SetPos((w / 2) - (wz / 2), (h / 2) - (hz / 2));

	// Hack...
	Panel *pCloseButton = FindChildByName("frame_close");
	if (pCloseButton)
	{
	...
	m_pOK->SetPos(x, y + hz + scheme()->GetProportionalScaledValue(2));

	m_pTitleLabel->SetSize(w, scheme()->GetProportionalScaledValue(24));
	m_pTitleLabel->SetPos(0, y - scheme()->GetProportionalScaledValue(25));

	SetTitle( m_szTitle, false );

	m_pTitleLabel->SetText( m_szTitle );
	m_pTitleLabel->SetContentAlignment(Label::Alignment::a_center);
