...
{	
	...
	m_bNeedsDraw = false;
	SetHiddenBits(HIDEHUD_MISCSTATUS | HIDEHUD_INWEPSELECTION);
}
...
void CHudHistoryResource::MsgFunc_ItemPickup( bf_read &msg )
{
	...
	if (iAmmoIndex >= 0)
		AddToHistory(HISTSLOT_AMMO, iAmmoIndex, 1);
	else
...
void CHudHistoryResource::Paint( void )
{
	...
	for ( int i = 0; i < m_PickupHistory.Count(); i++ )
	{
		if ( m_PickupHistory[i].type )
		{
			...
			int xpos = wide - icon_wide - m_flIconInset;
			...
			surface()->DrawTexturedRect(xpos, ypos, xpos + icon_wide, ypos + icon_height);

			if ( itemAmmoIcon )
			{
				surface()->DrawSetColor( clr );
				surface()->DrawSetTexture( itemAmmoIcon->textureId );
				surface()->DrawTexturedRect( xpos, ypos, xpos + icon_wide, ypos + icon_height );
			}
			...
			if ( iAmount )
			{
				...
				ypos -= (surface()->GetFontTall(m_hNumberFont) - icon_height) / 2;

				vgui::surface()->DrawSetTextFont( m_hNumberFont );
				vgui::surface()->DrawSetTextColor( Color( 255, 255, 255, 0 ) );
			...
			{
				// offset the number to sit properly next to the icon
				ypos -= (surface()->GetFontTall(m_hTextFont) - icon_height) / 2;
