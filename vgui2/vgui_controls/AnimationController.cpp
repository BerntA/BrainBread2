...
//-----------------------------------------------------------------------------
// Purpose: Check if a certain panel is being animated at this time.
//-----------------------------------------------------------------------------
bool AnimationController::IsPanelBeingAnimated(Panel *pPanel)
{
	for (int i = (m_ActiveAnimations.Count() - 1); i >= 0; i--)
	{
		if (m_ActiveAnimations[i].panel.Get() == pPanel)
			return true;
	}
