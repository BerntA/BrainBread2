...
ScrollBar::ScrollBar(Panel *parent, const char *panelName, bool vertical) : Panel(parent, panelName)
{
	...
	UseImages("scrollbar/button_up", "scrollbar/button_down", "scrollbar/base", "scrollbar/dragger");
...
void ScrollBar::UpdateButtonsForImages( void )
{
	...
	if ( m_pUpArrow || m_pDownArrow )
	{
		...
		_button[0]->SetEnabled(true);
		_button[0]->SetPaintBorderEnabled(false);
		_button[0]->SetPaintBackgroundEnabled(false);

		_button[1]->SetEnabled(true);
		_button[1]->SetPaintBorderEnabled(false);
		_button[1]->SetPaintBackgroundEnabled(false);

		_button[0]->SetZPos(10);
		_button[1]->SetZPos(10);
